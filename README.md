...
A Python environment is exported through `GITHUB_PATH` and `VIRTUAL_ENV`, so later steps render with it without sourcing anything.
...
| Detected                        | Installed                                                                                  |
| ------------------------------- | ------------------------------------------------------------------------------------------ |
| `knitr`                         | R (public RSPM binaries), then `renv` when a `renv.lock` is found, otherwise `knitr` and `rmarkdown`. |
| `jupyter`                       | `uv` and Python, then `uv sync` when a `pyproject.toml` is found, otherwise a bootstrap environment with `jupyter` and `papermill`. |
| `julia`                         | Julia, then `Pkg.instantiate()` and `IJulia`.                                              |
